...
-- File: rv16agu.vhd
--------------------------------------------------------------------------------
-- address generator unit, calculate pointer for load & store memory access
...
--------------------------------------------------------------------------------
entity rv16agu is generic(XLEN : natural := 32);
port (
...
--------------------------------------------------------------------------------
architecture rtl of rv16agu is

signal s_ins : std_logic_vector(31 downto 0); -- instruction
signal s_sgn, s_i_type, s_s_type : std_logic_vector(XLEN-1 downto 0);
...
begin
  ...
  s_i_type <= s_sgn(XLEN-1 downto 12)& s_ins(31 downto 25)& s_ins(24 downto 20); -- I-Type
  s_s_type <= s_sgn(XLEN-1 downto 12)& s_ins(31 downto 25)& s_ins(11 downto  7); -- S-Type
  
  s_agu    <= signed(s_rs1) + signed(s_s_type(XLEN-1 downto 0)) when(s_ins(5) = '1') -- store
         else signed(s_rs1) + signed(s_i_type(XLEN-1 downto 0));                     -- load
